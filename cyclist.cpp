...
CycList CycList::operator+(const CycList& c)
{
    ...
    if(i) do
    {
        x.pushLast(i->index,i->value);
        i=i->next;
    }while(i!=this->first);
    if(j) do
    {
        x.pushLast(j->index,j->value);
        j=j->next;
    }while(j!=c.first);
    return x;
}
...
CycList CycList::operator-(CycList& c)
{
    ...
    if(this->quantity() > c.quantity())
    {
        ...
        if(j) do
        {
            n.pushLast(i->index,i->value-j->value);
            i=i->next;
            j=j->next;
        }while(j!=c.first);
        do
        {
            n.pushLast(i->index,i->value);
            i=i->next;
        }while(i!=this->first);
    }//this>=c
    if(this->quantity()==c.quantity())
